defmodule Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogramTest do
  ...
  defp datapoint(tab, opts) do
    [dp] =
      Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.collect(
        tab,
        {"histogram", @scope},
        opts
      )
  ...
  describe "map_to_index/2" do
    ...
    test "exact powers of two follow the closed-form formula across scales" do
      ...
      assert Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.map_to_index(1.0, 0) ==
               -1

      assert Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.map_to_index(2.0, 0) ==
               0

      assert Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.map_to_index(4.0, 0) ==
               1

      assert Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.map_to_index(8.0, 0) ==
               2

      # Scale 1: indices double-step.
      assert Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.map_to_index(2.0, 1) ==
               1

      assert Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.map_to_index(4.0, 1) ==
               3
    end

    test "non-power-of-two values fall via the log path" do
      # 1 < 1.5 < 2 → bucket 0 at scale 0.
      assert Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.map_to_index(1.5, 0) ==
               0

      assert Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.map_to_index(3.0, 0) ==
               1

      # Scale 3 example from spec L626 — 1.09051 ≈ 2**(1/8) hits bucket 1.
      assert Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.map_to_index(1.09051, 3) ==
               1
    end

    test "values < 1 produce negative indices" do
      assert Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.map_to_index(0.5, 0) ==
               -2

      assert Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.map_to_index(0.25, 0) ==
               -3
    end
  end
  ...
  describe "aggregate/4 — value placement by sign" do
    test "zeros increment zero_count; non-zero values populate positive/negative buckets",
      ...
      for v <- [0, 0.0, 5, -4.0, -8.0] do
        Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.aggregate(
          tab,
          key(),
          v,
          opts
  ...
  test "record_min_max: false leaves min/max nil", %{tab: tab} do
    ...
    Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.aggregate(tab, key(), 1.0, opts)
  ...
  test "datapoint exposes proto-shaped fields", %{tab: tab, opts: opts} do
    Otel.SDK.Metrics.Aggregation.Base2ExponentialBucketHistogram.aggregate(tab, key(), 4.0, opts)
