defmodule Otel.SDK.Metrics.Aggregation.SumTest do
  ...
  describe "collect/3" do
    test "returns one datapoint per attribute set; combines int+float into a single value" do
      ...
      Otel.SDK.Metrics.Aggregation.Sum.aggregate(tab, key(%{"m" => "GET"}), 10, %{})
      Otel.SDK.Metrics.Aggregation.Sum.aggregate(tab, key(%{"m" => "GET"}), 2.5, %{})
      Otel.SDK.Metrics.Aggregation.Sum.aggregate(tab, key(%{"m" => "POST"}), 5, %{})
