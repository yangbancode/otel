defmodule Otel.SDK.Trace.TracerTest do
  ...
  describe "enabled?/2" do
    test "false when no SpanProcessors are registered (any opts)", %{tracer: tracer} do
      refute Otel.SDK.Trace.Tracer.enabled?(tracer)
      refute Otel.SDK.Trace.Tracer.enabled?(tracer, span_name: "test")
    end
    ...
    test "true when at least one SpanProcessor is registered" do
      ...
      assert Otel.SDK.Trace.Tracer.enabled?({Otel.SDK.Trace.Tracer, %{processors_key: key}})
    end
  end
end
