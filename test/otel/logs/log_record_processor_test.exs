defmodule Otel.Logs.LogRecordProcessorTest do
  ...
  defp emit(config, body) do
    Otel.Logs.LogRecordProcessor.on_emit(
      %Otel.Logs.LogRecord{body: body},
      %{},
      config
    )
  end
  ...
  test "enabled?/4 always returns true" do
    assert Otel.Logs.LogRecordProcessor.enabled?(%{}, %{}, [], %{})
  end
  ...
  describe "force_flush/1 + shutdown/1" do
    test "force_flush exports queued records and invokes exporter.force_flush" do
      ...
      assert :ok = Otel.Logs.LogRecordProcessor.force_flush(config)
    ...
    test "shutdown drains queue, calls exporter force_flush + shutdown in order" do
      ...
      assert :ok = Otel.Logs.LogRecordProcessor.shutdown(config)
    ...
    test "second shutdown / force_flush after first → {:error, :already_shutdown}" do
      config = start_batch({TestExporter, %{test_pid: self()}})
      :ok = Otel.Logs.LogRecordProcessor.shutdown(config)

      assert {:error, :already_shutdown} =
               Otel.Logs.LogRecordProcessor.shutdown(config)

      assert {:error, :already_shutdown} =
               Otel.Logs.LogRecordProcessor.force_flush(config)
    end
  end
  ...
  describe "caller-supplied timeout (spec L487-L491)" do
    test "force_flush/2 + shutdown/2 → {:error, :timeout} when budget exceeded" do
      ...
      assert {:error, :timeout} =
               Otel.Logs.LogRecordProcessor.force_flush(flush_cfg, 50)

      shut_cfg = start_batch(slow_exporter)
      emit(shut_cfg, "slow")

      assert {:error, :timeout} =
               Otel.Logs.LogRecordProcessor.shutdown(shut_cfg, 50)
    end
    ...
    test "force_flush/2 + shutdown/2 wait for an in-flight runner when the deadline allows" do
      ...
      assert :ok = Otel.Logs.LogRecordProcessor.force_flush(flush_cfg, 5_000)
      assert_receive :exported_after_delay, 500

      shut_cfg = start_batch({SlowExporter, %{delay_ms: 50, test_pid: self()}})
      emit(shut_cfg, "queued")

      assert :ok = Otel.Logs.LogRecordProcessor.shutdown(shut_cfg, 5_000)
      assert_receive :exported_after_delay, 500
    end
    ...
    test "force_flush/2 + shutdown/2 abort an in-flight runner whose remaining time exceeds the deadline" do
      ...
      emit(flush_cfg, "slow")
      Task.async(fn -> Otel.Logs.LogRecordProcessor.force_flush(flush_cfg, 5_000) end)
      Process.sleep(20)

      started = System.monotonic_time(:millisecond)

      assert {:error, :timeout} =
               Otel.Logs.LogRecordProcessor.force_flush(flush_cfg, 100)

      assert System.monotonic_time(:millisecond) - started < 1_000

      shut_cfg = start_batch(slow_5s)
      emit(shut_cfg, "slow")
      Task.async(fn -> Otel.Logs.LogRecordProcessor.force_flush(shut_cfg, 5_000) end)
      Process.sleep(20)

      started = System.monotonic_time(:millisecond)
      assert {:error, :timeout} = Otel.Logs.LogRecordProcessor.shutdown(shut_cfg, 100)
      assert System.monotonic_time(:millisecond) - started < 1_000
    end
    ...
    test "force_flush/2 with timeout: 0 takes the immediate-deadline branch in :exporting" do
      ...
      emit(config, "slow")
      Task.async(fn -> Otel.Logs.LogRecordProcessor.force_flush(config, 5_000) end)
      Process.sleep(20)

      assert {:error, :timeout} =
               Otel.Logs.LogRecordProcessor.force_flush(config, 0)
    end
