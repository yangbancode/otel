defmodule Otel.Metrics.Exemplar.Reservoir.SimpleFixedSizeTest do
  use ExUnit.Case, async: true

  defp new_state(opts \\ %{}),
    do: Otel.Metrics.Exemplar.Reservoir.SimpleFixedSize.new(opts)

  defp offer(state, value),
    do: Otel.Metrics.Exemplar.Reservoir.SimpleFixedSize.offer(state, value, 1000, %{}, %{})
  ...
  describe "collect/1" do
    test "returns the offered exemplars and resets count + storage" do
      ...
      assert {[_, _], %{count: 0, exemplars: %{}}} =
               Otel.Metrics.Exemplar.Reservoir.SimpleFixedSize.collect(state)
    end

    test "empty reservoir → []" do
      assert {[], _} =
               Otel.Metrics.Exemplar.Reservoir.SimpleFixedSize.collect(new_state())
    end
  end
end
