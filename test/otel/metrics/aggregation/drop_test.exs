defmodule Otel.Metrics.Aggregation.DropTest do
  ...
  test "aggregate/4 stores nothing; collect/3 always returns []" do
    ...
    assert :ok = Otel.Metrics.Aggregation.Drop.aggregate(tab, {:k, :s, %{}}, 42, %{})
    assert :ets.tab2list(tab) == []
    assert [] = Otel.Metrics.Aggregation.Drop.collect(tab, {"name", :scope}, %{})
  end
end
