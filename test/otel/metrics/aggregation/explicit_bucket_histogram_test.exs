defmodule Otel.Metrics.Aggregation.ExplicitBucketHistogramTest do
  ...
  test "default_boundaries/0 returns the OTel-spec defaults" do
    assert Otel.Metrics.Aggregation.ExplicitBucketHistogram.default_boundaries() ==
  ...
  describe "aggregate/4 + collect/3" do
    ...
    test "tracks float sum precisely", %{tab: tab, opts: opts} do
      Otel.Metrics.Aggregation.ExplicitBucketHistogram.aggregate(tab, key(), 1.5, opts)
      Otel.Metrics.Aggregation.ExplicitBucketHistogram.aggregate(tab, key(), 2.5, opts)
    ...
    test "keeps separate datapoints per attribute set", %{tab: tab, opts: opts} do
      Otel.Metrics.Aggregation.ExplicitBucketHistogram.aggregate(
        tab,
        key(%{"m" => "GET"}),
        5,
        opts
