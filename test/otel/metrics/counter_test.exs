defmodule Otel.Metrics.CounterTest do
  ...
  describe "create/3 — delegates to Meter.create_counter" do
    test "with name only" do
      assert %Otel.Metrics.Instrument{kind: :counter, name: "n"} =
               Otel.Metrics.Counter.create("n")
    end

    test "forwards unit, description, and advisory opts" do
  ...
  test "add/3 returns :ok across value shapes and attribute payloads" do
    inst = Otel.Metrics.Counter.create("n")
