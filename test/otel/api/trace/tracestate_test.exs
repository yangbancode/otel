defmodule Otel.API.Trace.TraceStateTest do
  ...
  describe "empty state" do
    test "default struct is empty and encodes to \"\"" do
      assert Otel.API.Trace.TraceState.empty?(%Otel.API.Trace.TraceState{})
  ...
  describe "get/2" do
    test "returns the value for an existing key, \"\" for a missing one" do
      ts = Otel.API.Trace.TraceState.add(%Otel.API.Trace.TraceState{}, "vendor", "value")
  ...
  describe "add/3" do
    test "prepends new entries (left-most = newest)" do
      ts =
        ...
        |> Otel.API.Trace.TraceState.add("existing", "data")
        |> Otel.API.Trace.TraceState.add("new", "value")

      assert Otel.API.Trace.TraceState.encode(ts) == "new=value,existing=data"
    end
    ...
    test "rejects a duplicate key (preserves the first value)" do
      ts =
        %Otel.API.Trace.TraceState{}
        |> Otel.API.Trace.TraceState.add("vendor", "first")
        |> Otel.API.Trace.TraceState.add("vendor", "second")

      assert Otel.API.Trace.TraceState.get(ts, "vendor") == "first"
    end
    ...
    test "drops the right-most (oldest) entry when at the 32-member cap" do
      ts =
        Enum.reduce(1..32, %Otel.API.Trace.TraceState{}, fn i, acc ->
          Otel.API.Trace.TraceState.add(acc, "key#{i}", "val#{i}")
        end)
        |> Otel.API.Trace.TraceState.add("extra", "value")

      assert Otel.API.Trace.TraceState.get(ts, "extra") == "value"
      # Right-most (oldest = "key1") dropped; second-oldest retained.
      assert Otel.API.Trace.TraceState.get(ts, "key1") == ""
      assert Otel.API.Trace.TraceState.get(ts, "key2") == "val2"
    end
  end
  ...
  describe "update/3" do
    test "updates an existing entry and moves it to the front" do
      ts =
        %Otel.API.Trace.TraceState{}
        |> Otel.API.Trace.TraceState.add("c", "3")
        |> Otel.API.Trace.TraceState.add("b", "2")
        |> Otel.API.Trace.TraceState.add("a", "1")
        |> Otel.API.Trace.TraceState.update("b", "updated")

      assert Otel.API.Trace.TraceState.encode(ts) == "b=updated,a=1,c=3"
    end
    ...
    test "missing key falls through to add semantics, respecting the 32-cap" do
      ...
      ts = Otel.API.Trace.TraceState.update(base, "missing", "v")

      assert Otel.API.Trace.TraceState.get(ts, "missing") == "v"
  ...
  describe "delete/2" do
    test "removes the key; no-op for missing key" do
      ...
      assert Otel.API.Trace.TraceState.delete(ts, "a")
             |> Otel.API.Trace.TraceState.encode() == "b=2"

      assert Otel.API.Trace.TraceState.delete(ts, "missing")
             |> Otel.API.Trace.TraceState.encode() == "a=1,b=2"
    end
  end
  ...
  describe "encode/1 + decode/1 (W3C wire format)" do
    test "round-trips canonical entries" do
      ...
      assert header
             |> Otel.API.Trace.TraceState.decode()
             |> Otel.API.Trace.TraceState.encode() == header
    end

    test "decode tolerates whitespace, empty list-members, and dedupes (last wins)" do
      ts = Otel.API.Trace.TraceState.decode(" a=1 , , b=2 , a=last ")

      # Last occurrence wins on dedup.
      assert Otel.API.Trace.TraceState.get(ts, "a") == "last"
      assert Otel.API.Trace.TraceState.get(ts, "b") == "2"
    end

    test "decode \"\" yields empty TraceState" do
      assert Otel.API.Trace.TraceState.empty?(Otel.API.Trace.TraceState.decode(""))
    end
  end
  ...
  describe "decode parsing — validation deferred to mutation" do
    ...
    test "preserves entries with otherwise-invalid keys" do
      ...
      assert Otel.API.Trace.TraceState.get(ts, "BAD") == "v"
  ...
  describe "valid_key?/1 — W3C §3.3.1.3.1" do
    test "true for the documented allowed shapes" do
      ...
      assert Otel.API.Trace.TraceState.valid_key?("my-key_1*2/3@system")
      # Level 2: digit-leading and multi-tenant.
      assert Otel.API.Trace.TraceState.valid_key?("1vendor")
      assert Otel.API.Trace.TraceState.valid_key?("tenant@system")
      assert Otel.API.Trace.TraceState.valid_key?("a@b@c")
    end
  ...
  test "add/update silently drop entries that fail validation" do
    base = Otel.API.Trace.TraceState.add(%Otel.API.Trace.TraceState{}, "a", "1")

    assert Otel.API.Trace.TraceState.add(base, "BAD", "v") == base
    assert Otel.API.Trace.TraceState.add(base, "good", "a,b") == base
    assert Otel.API.Trace.TraceState.update(base, "BAD", "v") == base
    assert Otel.API.Trace.TraceState.update(base, "a", "a,b") == base
  end

  test "all mutation operations return new structs (immutability)" do
    ts1 = Otel.API.Trace.TraceState.add(%Otel.API.Trace.TraceState{}, "a", "1")
    ts2 = Otel.API.Trace.TraceState.add(ts1, "b", "2")
    ts3 = Otel.API.Trace.TraceState.delete(ts2, "a")

    assert Otel.API.Trace.TraceState.encode(ts1) == "a=1"
    assert Otel.API.Trace.TraceState.encode(ts2) == "b=2,a=1"
    assert Otel.API.Trace.TraceState.encode(ts3) == "b=2"
  end
end
