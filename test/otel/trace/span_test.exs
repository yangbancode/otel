defmodule Otel.Trace.SpanTest do
  ...
  @module_key {Otel.Trace.Span, :module}
  ...
  # (`Otel.Trace.Span.Impl` under normal boot) on exit. Without
  ...
  describe "Noop fallback (no module registered)" do
    ...
    # `Otel.Trace.Span.NoopTest`. These tests only assert
    # that the facade reaches Noop when nothing is registered.
    test "recording?/1 returns false (Noop default)" do
      assert Otel.Trace.Span.recording?(@valid_ctx) == false
    end

    test "set_status/2 returns :ok (Noop default)" do
      assert Otel.Trace.Span.set_status(@valid_ctx, Otel.Trace.Status.new(:error)) == :ok
    end
  end

  describe "set_module/1 + facade dispatch" do
    setup do
      Otel.Trace.Span.set_module(TagModule)
      :ok
    end

    test "recording?/1" do
      assert Otel.Trace.Span.recording?(@valid_ctx) == {:recording?, @valid_ctx}
    end

    test "set_attribute/3" do
      assert Otel.Trace.Span.set_attribute(@valid_ctx, "k", "v") ==
    ...
    test "set_attributes/2" do
      ...
      assert Otel.Trace.Span.set_attributes(@valid_ctx, attrs) ==
               {:set_attributes, @valid_ctx, attrs}
    end

    test "add_event/2" do
      event = Otel.Trace.Event.new("e")
      assert Otel.Trace.Span.add_event(@valid_ctx, event) == {:add_event, @valid_ctx, event}
    end

    test "add_link/2" do
      link = %Otel.Trace.Link{context: Otel.Trace.SpanContext.new(0xAA, 0xBB)}
      assert Otel.Trace.Span.add_link(@valid_ctx, link) == {:add_link, @valid_ctx, link}
    end

    test "set_status/2" do
      status = Otel.Trace.Status.new(:ok)

      assert Otel.Trace.Span.set_status(@valid_ctx, status) ==
               {:set_status, @valid_ctx, status}
    end

    test "update_name/2" do
      assert Otel.Trace.Span.update_name(@valid_ctx, "new") ==
               {:update_name, @valid_ctx, "new"}
    end

    test "end_span/1 substitutes a default timestamp" do
      assert {:end_span, @valid_ctx, ts} = Otel.Trace.Span.end_span(@valid_ctx)
      assert is_integer(ts) and ts > 0
    end

    test "end_span/2 forwards explicit timestamp" do
      assert Otel.Trace.Span.end_span(@valid_ctx, 1_000_000) ==
    ...
    test "record_exception/2 defaults stacktrace and attributes" do
      ...
      assert Otel.Trace.Span.record_exception(@valid_ctx, ex) ==
    ...
    test "record_exception/3 forwards stacktrace, defaults attributes" do
      ...
      assert Otel.Trace.Span.record_exception(@valid_ctx, ex, st) ==
    ...
    test "record_exception/4 forwards everything" do
      ...
      assert Otel.Trace.Span.record_exception(@valid_ctx, ex, st, attrs) ==
