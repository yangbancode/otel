defmodule Otel.Trace.Span.ImplOperationsTest do
  ...
  describe "recording?/1" do
    test "true while alive; false after end_span; false for unknown span_id" do
      span_ctx = start_span()
      assert Otel.Trace.Span.Impl.recording?(span_ctx) == true

      Otel.Trace.Span.Impl.end_span(span_ctx, nil)
      assert Otel.Trace.Span.Impl.recording?(span_ctx) == false

      assert Otel.Trace.Span.Impl.recording?(%Otel.Trace.SpanContext{span_id: 999_999}) ==
               false
    end
  end
  ...
  describe "set_attribute/3 + set_attributes/2" do
    test "set / overwrite / no-op-after-end / count_limit / overwrite-at-limit" do
      ...
      Otel.Trace.Span.Impl.set_attribute(span_ctx, "key", "new")
      Otel.Trace.Span.Impl.set_attribute(span_ctx, "extra", 1)
      assert stored(span_ctx).attributes == %{"key" => "new", "extra" => 1}

      Otel.Trace.Span.Impl.end_span(span_ctx, nil)
      assert :ok = Otel.Trace.Span.Impl.set_attribute(span_ctx, "k", "v")

      capped = start_span(span_limits: %Otel.Trace.SpanLimits{attribute_count_limit: 2})

      for {k, v} <- [{"a", 1}, {"b", 2}, {"c", 3}, {"d", 4}],
          do: Otel.Trace.Span.Impl.set_attribute(capped, k, v)
      ...
      Otel.Trace.Span.Impl.set_attribute(one_slot, "a", 1)
      Otel.Trace.Span.Impl.set_attribute(one_slot, "a", 2)
    ...
    test "value-length limit truncates strings (incl. inside arrays); non-strings unchanged; :infinity skips" do
      ...
      Otel.Trace.Span.Impl.set_attribute(tight, "num", 12_345)
      ...
      Otel.Trace.Span.Impl.set_attribute(infinite, "key", long)
    ...
    test "set_attributes/2 — accepts map and keyword list; honours count_limit; no-op after end" do
      span_ctx = start_span(attributes: %{"key" => "old"})
      Otel.Trace.Span.Impl.set_attributes(span_ctx, %{"a" => 1, "b" => 2, "key" => "new"})
      assert stored(span_ctx).attributes == %{"a" => 1, "b" => 2, "key" => "new"}

      kw_span = start_span()
      Otel.Trace.Span.Impl.set_attributes(kw_span, [{"a", 1}, {"b", 2}])
      assert stored(kw_span).attributes == %{"a" => 1, "b" => 2}

      capped = start_span(span_limits: %Otel.Trace.SpanLimits{attribute_count_limit: 2})
      Otel.Trace.Span.Impl.set_attributes(capped, %{"a" => 1, "b" => 2, "c" => 3, "d" => 4})
      assert map_size(stored(capped).attributes) == 2
      assert stored(capped).dropped_attributes_count == 2

      Otel.Trace.Span.Impl.end_span(span_ctx, nil)
      assert :ok = Otel.Trace.Span.Impl.set_attributes(span_ctx, %{"k" => "v"})
    end
  end
  ...
  describe "add_event/2" do
    test "appends events in insertion order; preserves attributes + custom timestamp" do
      ...
      Otel.Trace.Span.Impl.add_event(span_ctx, Otel.Trace.Event.new("first"))

      Otel.Trace.Span.Impl.add_event(
        span_ctx,
    ...
    test "event_count_limit + attribute_per_event_limit + per-event value-length truncation" do
      ...
      Otel.Trace.Span.Impl.add_event(capped, Otel.Trace.Event.new("dropped"))
      ...
      ended = start_span()
      Otel.Trace.Span.Impl.end_span(ended, nil)
      assert :ok = Otel.Trace.Span.Impl.add_event(ended, Otel.Trace.Event.new("late"))
    end
  end
  ...
  describe "add_link/2" do
    test "stores link with attributes; honours link_count + attribute_per_link + value-length limits" do
      ...
      Otel.Trace.Span.Impl.add_link(span_ctx, %Otel.Trace.Link{
        context: linked,
        attributes: %{"key" => "val"}
      })
      ...
      assert :ok =
               Otel.Trace.Span.Impl.add_link(ended, %Otel.Trace.Link{
  ...
  test "set_status/2 follows Ok > Error > Unset precedence" do
    ...
    Otel.Trace.Span.Impl.set_status(error_then_unset, Otel.Trace.Status.new(:error, "fail"))
    Otel.Trace.Span.Impl.set_status(error_then_unset, Otel.Trace.Status.new(:unset))

    assert stored(error_then_unset).status ==
             %Otel.Trace.Status{code: :error, description: "fail"}

    ok_then_error = start_span()
    Otel.Trace.Span.Impl.set_status(ok_then_error, Otel.Trace.Status.new(:ok))
    Otel.Trace.Span.Impl.set_status(ok_then_error, Otel.Trace.Status.new(:error, "fail"))

    assert stored(ok_then_error).status == %Otel.Trace.Status{code: :ok, description: ""}

    error_then_ok = start_span()
    Otel.Trace.Span.Impl.set_status(error_then_ok, Otel.Trace.Status.new(:error, "fail"))
    Otel.Trace.Span.Impl.set_status(error_then_ok, Otel.Trace.Status.new(:ok))
    ...
    assert :ok =
             Otel.Trace.Span.Impl.set_status(ended, Otel.Trace.Status.new(:error, "late"))
  end

  test "update_name/2 mutates while alive; no-op after end" do
    span_ctx = start_span()
    Otel.Trace.Span.Impl.update_name(span_ctx, "new_name")
    assert stored(span_ctx).name == "new_name"

    Otel.Trace.Span.Impl.end_span(span_ctx, nil)
    assert :ok = Otel.Trace.Span.Impl.update_name(span_ctx, "ignored")
  end
  ...
  describe "end_span/2" do
    test "removes span from ETS; sets is_recording=false; uses given timestamp; calls every processor; second end is a no-op" do
      ...
      :ok = Otel.Trace.Span.Impl.end_span(span_ctx, 42)
      ...
      assert :ok = Otel.Trace.Span.Impl.end_span(span_ctx, nil)
    end
    ...
    test "end_span(_, nil) records System.system_time(:nanosecond)" do
      ...
      :ok = Otel.Trace.Span.Impl.end_span(span_ctx, nil)
    ...
    test "logs \"span limits applied\" on end when any limit triggered; silent otherwise" do
      ...
      log = capture_log(fn -> Otel.Trace.Span.Impl.end_span(capped, nil) end)
      assert log =~ "span limits applied"
      assert log =~ "dropped 2 attributes"

      clean = start_span()
      Otel.Trace.Span.Impl.set_attribute(clean, "a", 1)
      log = capture_log(fn -> Otel.Trace.Span.Impl.end_span(clean, nil) end)
  ...
  test "record_exception/4 — adds 'exception' event with type/message/stacktrace + extra attrs" do
    ...
    Otel.Trace.Span.Impl.record_exception(
      span_ctx,
