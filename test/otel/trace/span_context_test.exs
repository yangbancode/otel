defmodule Otel.Trace.SpanContextTest do
  ...
  test "new/0 has all zeros, empty TraceState, is_remote false" do
    assert Otel.Trace.SpanContext.new() ==
  ...
  describe "new/1" do
    test "trace_flags and tracestate default to 0 / empty" do
      ctx = Otel.Trace.SpanContext.new(%{trace_id: @trace_id, span_id: @span_id})
  ...
  describe "valid?/1" do
    ...
    test "true only when both trace_id and span_id are non-zero" do
      assert Otel.Trace.SpanContext.valid?(
               Otel.Trace.SpanContext.new(%{trace_id: @trace_id, span_id: @span_id})
             )
    end

    test "false when either id is zero" do
      refute Otel.Trace.SpanContext.valid?(
               Otel.Trace.SpanContext.new(%{trace_id: 0, span_id: @span_id})
             )

      refute Otel.Trace.SpanContext.valid?(
               Otel.Trace.SpanContext.new(%{trace_id: @trace_id, span_id: 0})
             )

      refute Otel.Trace.SpanContext.valid?(Otel.Trace.SpanContext.new())
    end
  end
