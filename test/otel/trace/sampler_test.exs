defmodule Otel.Trace.SamplerTest do
  ...
  describe "description/0" do
    test "returns the spec-style ParentBased descriptor with all five branches" do
      assert Otel.Trace.Sampler.description() ==
  ...
  describe "should_sample/6 — root span (no parent)" do
    test "samples the root span" do
      ...
      assert {:record_and_sample, %{}, %Otel.Trace.TraceState{}} =
               Otel.Trace.Sampler.should_sample(ctx, @trace_id, [], "root", :internal, %{})
    end
  end
  ...
  describe "should_sample/6 — tracestate propagation" do
    test "carries the parent tracestate through unchanged" do
      tracestate =
        Otel.Trace.TraceState.new() |> Otel.Trace.TraceState.add("vendor", "value")

      parent = %Otel.Trace.SpanContext{
        trace_id: @trace_id,
      ...
      ctx = Otel.Trace.set_current_span(Otel.Ctx.new(), parent)

      assert {:record_and_sample, %{}, ^tracestate} =
               Otel.Trace.Sampler.should_sample(ctx, @trace_id, [], "child", :internal, %{})
    end
  end
