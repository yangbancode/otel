defmodule Otel.Configuration.SubstitutionTest do
  ...
  describe "basic substitution" do
    test "${VAR} substitutes a defined env var" do
      ...
      assert Otel.Configuration.Substitution.substitute!("${OTEL_TEST_FOO}") == "hello"
    end
    ...
    test "${VAR} returns empty string when defined but empty" do
      ...
      assert Otel.Configuration.Substitution.substitute!("${OTEL_TEST_EMPTY}") == ""
    end

    test "${env:VAR} works the same as ${VAR}" do
      System.put_env("OTEL_TEST_FOO", "hello")
      assert Otel.Configuration.Substitution.substitute!("${env:OTEL_TEST_FOO}") == "hello"
    end
  end
  ...
  describe "$$ escape" do
    ...
    test "spec pseudocode example: '$${FOO} ${BAR} $${BAZ}' (data-model.md L368-L376)" do
      ...
      assert Otel.Configuration.Substitution.substitute!(input) == expected
    end
  end
  ...
  describe "literal characters pass through" do
    test "input without any substitution is returned unchanged" do
      assert Otel.Configuration.Substitution.substitute!("hello world: 42") == "hello world: 42"
    end

    test "lone $ (not followed by $ or {) is preserved" do
      assert Otel.Configuration.Substitution.substitute!("price: $42") == "price: $42"
    end
  end
