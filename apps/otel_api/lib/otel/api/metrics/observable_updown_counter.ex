defmodule Otel.API.Metrics.ObservableUpDownCounter do
  @moduledoc """
  Asynchronous UpDownCounter instrument.

  Reports additive absolute value(s) that can increase or decrease
  when the instrument is being observed. Created exclusively through
  a Meter.

  Unlike `UpDownCounter.add`, the callback reports the absolute value —
  the SDK derives rate of change by differencing successive readings.
  If values are monotonically increasing, use `ObservableCounter` instead.
  ...
  @doc """
  Creates an ObservableUpDownCounter without an inline callback.
