defmodule Otel.API.Metrics.InstrumentTest do
  ...
  describe "validate_name/1" do
    test "valid simple name" do
      assert {:ok, "requests"} = Otel.API.Metrics.Instrument.validate_name("requests")
    end

    test "valid name with allowed special chars" do
      assert {:ok, "http.server.request_duration"} =
               Otel.API.Metrics.Instrument.validate_name("http.server.request_duration")
    end

    test "valid name with dash and slash" do
      assert {:ok, "my-lib/counter"} =
               Otel.API.Metrics.Instrument.validate_name("my-lib/counter")
    end

    test "valid single char name" do
      assert {:ok, "a"} = Otel.API.Metrics.Instrument.validate_name("a")
    end

    test "valid 255 char name" do
      name = "a" <> String.duplicate("b", 254)
      assert {:ok, ^name} = Otel.API.Metrics.Instrument.validate_name(name)
    end

    test "nil name returns error" do
      assert {:error, _} = Otel.API.Metrics.Instrument.validate_name(nil)
    end

    test "empty name returns error" do
      assert {:error, _} = Otel.API.Metrics.Instrument.validate_name("")
    end

    test "name starting with digit returns error" do
      assert {:error, _} = Otel.API.Metrics.Instrument.validate_name("1counter")
    end

    test "name starting with underscore returns error" do
      assert {:error, _} = Otel.API.Metrics.Instrument.validate_name("_counter")
    end

    test "name exceeding 255 chars returns error" do
      name = "a" <> String.duplicate("b", 255)
      assert {:error, _} = Otel.API.Metrics.Instrument.validate_name(name)
    end

    test "name with invalid char returns error" do
      assert {:error, _} = Otel.API.Metrics.Instrument.validate_name("my counter")
    end
  end
