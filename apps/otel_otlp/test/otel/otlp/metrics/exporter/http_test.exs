defmodule Otel.OTLP.Metrics.Exporter.HTTPTest do
  use ExUnit.Case
  ...
  describe "init/1 defaults" do
    test "returns state with default values" do
      {:ok, state} = Otel.OTLP.Metrics.Exporter.HTTP.init(%{})
    ...
    test "code config overrides defaults" do
      {:ok, state} =
        Otel.OTLP.Metrics.Exporter.HTTP.init(%{
  ...
  describe "init/1 OTEL_EXPORTER_OTLP_METRICS_HEADERS" do
    ...
    test "code config headers as map" do
      {:ok, state} = Otel.OTLP.Metrics.Exporter.HTTP.init(%{headers: %{"auth" => "token"}})
  ...
  describe "init/1 SSL" do
    ...
    test "https endpoint gets default ssl_options" do
      {:ok, state} = Otel.OTLP.Metrics.Exporter.HTTP.init(%{endpoint: "https://collector:4318"})
  ...
  describe "export/2 success" do
    test "returns :ok for empty metrics list" do
      {:ok, state} = Otel.OTLP.Metrics.Exporter.HTTP.init(%{})
      assert Otel.OTLP.Metrics.Exporter.HTTP.export([], state) == :ok
    end

    test "returns :ok when server responds 200" do
      {pid, port, listen} = start_test_server(200)
      {:ok, state} = Otel.OTLP.Metrics.Exporter.HTTP.init(%{endpoint: "http://localhost:#{port}"})
      assert Otel.OTLP.Metrics.Exporter.HTTP.export([@test_metric], state) == :ok
  ...
  describe "force_flush/1 and shutdown/1" do
    test "force_flush returns :ok" do
      {:ok, state} = Otel.OTLP.Metrics.Exporter.HTTP.init(%{})
      assert Otel.OTLP.Metrics.Exporter.HTTP.force_flush(state) == :ok
    end

    test "shutdown returns :ok" do
      {:ok, state} = Otel.OTLP.Metrics.Exporter.HTTP.init(%{})
      assert Otel.OTLP.Metrics.Exporter.HTTP.shutdown(state) == :ok
    end
  end
end
