defmodule Otel.OTLP.Logs.Exporter.HTTPTest do
  use ExUnit.Case
  ...
  describe "init/1 defaults" do
    test "returns state with default values" do
      {:ok, state} = Otel.OTLP.Logs.Exporter.HTTP.init(%{})
  ...
  describe "init/1 endpoint" do
    ...
    test "code config overrides defaults" do
      {:ok, state} =
        Otel.OTLP.Logs.Exporter.HTTP.init(%{
    ...
    test "general env overrides code config" do
      ...
      {:ok, state} = Otel.OTLP.Logs.Exporter.HTTP.init(%{endpoint: "http://code:4318"})
  ...
  describe "init/1 headers" do
    ...
    test "code config headers as map" do
      {:ok, state} = Otel.OTLP.Logs.Exporter.HTTP.init(%{headers: %{"auth" => "token"}})
  ...
  describe "init/1 SSL" do
    ...
    test "https endpoint gets default ssl_options" do
      {:ok, state} = Otel.OTLP.Logs.Exporter.HTTP.init(%{endpoint: "https://collector:4318"})
  ...
  describe "export/2" do
    test "returns :ok for empty list" do
      {:ok, state} = Otel.OTLP.Logs.Exporter.HTTP.init(%{})
      assert :ok == Otel.OTLP.Logs.Exporter.HTTP.export([], state)
    end

    test "returns :ok when server responds 200" do
      {pid, port, listen} = start_test_server(200)
      {:ok, state} = Otel.OTLP.Logs.Exporter.HTTP.init(%{endpoint: "http://localhost:#{port}"})
      assert :ok == Otel.OTLP.Logs.Exporter.HTTP.export([@test_log_record], state)
  ...
  describe "shutdown/1 and force_flush/1" do
    test "shutdown returns :ok" do
      {:ok, state} = Otel.OTLP.Logs.Exporter.HTTP.init(%{})
      assert :ok == Otel.OTLP.Logs.Exporter.HTTP.shutdown(state)
    end

    test "force_flush returns :ok" do
      {:ok, state} = Otel.OTLP.Logs.Exporter.HTTP.init(%{})
      assert :ok == Otel.OTLP.Logs.Exporter.HTTP.force_flush(state)
    end
  end
end
