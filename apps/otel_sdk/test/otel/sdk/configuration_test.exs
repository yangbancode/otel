defmodule Otel.SDK.ConfigurationTest do
  ...
  describe "merge/2 without env vars" do
    test "returns default merged with app config" do
      config = Otel.SDK.Configuration.merge(@default_config, %{})
    ...
    test "app config overrides defaults" do
      ...
      config = Otel.SDK.Configuration.merge(@default_config, app)
  ...
  describe "OTEL_BSP_* batch processor" do
    test "sets bsp_config" do
      System.put_env("OTEL_BSP_SCHEDULE_DELAY", "1000")
      System.put_env("OTEL_BSP_MAX_QUEUE_SIZE", "4096")
      config = Otel.SDK.Configuration.merge(@default_config, %{})

      assert config.bsp_config.scheduled_delay_ms == 1000
      assert config.bsp_config.max_queue_size == 4096
    end

    test "sets all bsp fields" do
      System.put_env("OTEL_BSP_EXPORT_TIMEOUT", "5000")
      System.put_env("OTEL_BSP_MAX_EXPORT_BATCH_SIZE", "256")
      config = Otel.SDK.Configuration.merge(@default_config, %{})

      assert config.bsp_config.export_timeout_ms == 5000
      assert config.bsp_config.max_export_batch_size == 256
    end

    test "unparseable BSP value defaults to 0" do
      System.put_env("OTEL_BSP_SCHEDULE_DELAY", "not_a_number")
      config = Otel.SDK.Configuration.merge(@default_config, %{})
      assert config.bsp_config.scheduled_delay_ms == 0
    end

    test "empty BSP value treated as unset" do
      System.put_env("OTEL_BSP_SCHEDULE_DELAY", "")
      config = Otel.SDK.Configuration.merge(@default_config, %{})
      refute Map.has_key?(config, :bsp_config)
    end

    test "no bsp_config when no env vars set" do
      config = Otel.SDK.Configuration.merge(@default_config, %{})
      refute Map.has_key?(config, :bsp_config)
    end
  end
