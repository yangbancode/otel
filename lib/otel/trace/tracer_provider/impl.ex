defmodule Otel.Trace.TracerProvider do
  @moduledoc """
  SDK implementation of the `Otel.Trace.Tracer.BehaviourProvider`
  ...
  *"TracerProvider — all methods MUST be documented that
  ...
  @type processor_entry :: %{
          module: module(),
          pid: pid() | nil,
          callback_config: Otel.Trace.SpanProcessor.config()
        }
  ...
  **SDK** (OTel API MUST) — Get a Tracer
  ...
  @spec get_tracer(
          ...
          Otel.Trace.Tracer.Behaviour.t()
  @impl Otel.Trace.Tracer.BehaviourProvider
  def get_tracer(server, %Otel.InstrumentationScope{} = instrumentation_scope) do
    ...
    :exit, {:noproc, _} -> {Otel.Trace.Tracer.Noop, []}
  end
  ...
  **SDK** (introspection) — Returns the current configuration
  ...
  def init(user_config) do
    ...
    Otel.Trace.Tracer.BehaviourProvider.set_provider({__MODULE__, self_ref()})
    {:ok, config}
  end
  ...
  def handle_call({:get_tracer, _instrumentation_scope}, _from, %{shut_down: true} = config) do
    {:reply, {Otel.Trace.Tracer.Noop, []}, config}
  end
  ...
  def handle_call(
    ...
    {:reply, {Otel.Trace.Tracer, tracer_config}, config}
  end
  ...
  defp warn_processor_exited(module, pid, reason) do
    Logger.warning(
      "Otel.Trace.TracerProvider: SpanProcessor #{inspect(module)} " <>
