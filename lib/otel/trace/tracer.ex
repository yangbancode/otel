defmodule Otel.Trace.Tracer do
  @moduledoc """
  SDK implementation of the `Otel.Trace.Tracer.Behaviour` behaviour
  ...
  @behaviour Otel.Trace.Tracer.Behaviour

  @doc """
  **SDK** (OTel API MUST) — Span Creation
  (`trace/api.md` §Span Creation L378-L414).

  Delegates to `Otel.Trace.Span.Impl.start_span/4` for sampling
  ...
  @spec start_span(
          ctx :: Otel.Ctx.t(),
          tracer :: Otel.Trace.Tracer.Behaviour.t(),
          name :: String.t(),
  ...
  def start_span(ctx, {__MODULE__, config}, name, opts) do
    {span_ctx, span} =
      Otel.Trace.Span.Impl.start_span(ctx, name, config.span_limits, opts)
  ...
  **SDK** (OTel API MUST) — `Enabled`
  ...
  implemented (see `Otel.Trace.TracerProvider` `## Deferred
  Development-status features`); only the no-processors leg is
  honoured today.
  """
  @spec enabled?(
          tracer :: Otel.Trace.Tracer.Behaviour.t(),
          opts :: Otel.Trace.Tracer.Behaviour.enabled_opts()
        ) :: boolean()
  @impl true
  ...
  @spec run_on_start(
          span :: Otel.Trace.Span.Impl.t(),
          ctx :: Otel.Ctx.t(),
          processors :: [{module(), Otel.Trace.SpanProcessor.config()}]
        ) :: Otel.Trace.Span.Impl.t()
