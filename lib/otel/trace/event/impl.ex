defmodule Otel.Trace.Event.Impl do
  @moduledoc """
  ...
  `Otel.Trace.Event` at the moment limits are applied
  (`Otel.Trace.Span.Impl.add_event/2`,
  `Otel.Trace.Span.Impl.start_span/6` for events provided at
  creation time). Application code should not construct
  this struct directly — use `Otel.Trace.Event.new/3`
  and pass the result to `Otel.Trace.Span.add_event/2`.

  ## Why a separate SDK type?

  `Otel.Trace.Event` (api.md L520-L558) defines an Event
