defmodule Otel.Trace.Tracer.BehaviourProvider do
  @moduledoc """
  ...
  (`Otel.Trace.Tracer.Noop`, matching the
  ...
  @default_tracer {Otel.Trace.Tracer.Noop, []}
  ...
  `Otel.Trace.Tracer.Behaviour.t/0` and keeps the API decoupled from
  SDK internals (GenServer, Registry, etc.).

  `dispatcher_module` MUST implement the
  `Otel.Trace.Tracer.BehaviourProvider` behaviour.
  """
  @type t :: {module(), term()}
  ...
  **Application** (OTel API MUST) — "Get a Tracer" (`trace/api.md`
  ...
  def get_tracer(%Otel.InstrumentationScope{} = instrumentation_scope) do
    ...
    # `Otel.Trace.TracerProvider.warn_invalid_scope_name/1`.
  ...
  **SDK** (installation hook) — register the global
  ...
  implement the `Otel.Trace.Tracer.BehaviourProvider` behaviour.
  """
