defmodule Otel.Trace.Tracer.Noop do
  @moduledoc """
  ...
  - If the parent Context contains no Span, an empty non-recording
    ...
    `%Otel.Trace.SpanContext{}` default struct.

  `enabled?/2` returns `false` per spec L201-L213 (Enabled is a
  SHOULD API; a no-op tracer is never enabled).

  Registered as the default tracer by `Otel.Trace.Tracer.BehaviourProvider`
  ...
  **SDK** (Noop implementation) — `start_span/4` no-op per
  ...
  def start_span(ctx, _tracer, _name, _opts) do
    case Otel.Trace.current_span(ctx) do
      %Otel.Trace.SpanContext{trace_id: trace_id} = parent when trace_id != 0 ->
        parent
