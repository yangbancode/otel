defmodule Otel.Trace do
  ...
  Thin dispatcher over `Otel.Trace.Tracer.BehaviourProvider` and the
  ...
  **Application** (OTel API MAY) — dispatches to the Tracer's
  ...
  `Otel.Trace.Tracer.Behaviour.with_span/5` callback contract for
  ...
  **Application** (Convenience) — facade over
  `Otel.Trace.Tracer.BehaviourProvider`.

  Returns a Tracer for the given instrumentation scope.
  Equivalent to calling `Otel.Trace.Tracer.BehaviourProvider.get_tracer/1`
  ...
  def get_tracer(%Otel.InstrumentationScope{} = instrumentation_scope) do
    Otel.Trace.Tracer.BehaviourProvider.get_tracer(instrumentation_scope)
  end
end
