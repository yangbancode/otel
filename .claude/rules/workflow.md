...
7. **Quality checks** — run `mix clean` first to clear compile cache, then run in order:
   ...
   - `mix credo --strict`
   - `mix dialyzer`
8. **Update docs** — if the change touches an architectural decision, update or add a doc under `docs/architecture/`. Otherwise update the affected module's `@moduledoc` so the rationale stays with the code
